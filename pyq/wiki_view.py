...
def http_handler(request, post_id):
    if request.method == "POST":
        ...
        return post_wiki(request, post_id)
    elif request.method == "PUT":
        return edit_wiki(request, post_id)
    elif request.method == "DELETE":
        return delete_wiki(request, post_id)
...
@login_required
def delete_wiki(request, post_id):
    request_dict = {}
    if request.user.has_perm("wiki.delete_wiki"):
...
@login_required
def edit_wiki(request, post_id):
    response_dict = {}
    if request.method == "PUT":
        post = StaticQuestion.objects.get(id=post_id)
        if request.user.has_perm("wiki.edit_wiki"):
...
def post_static(request):
     if request.method == "POST":
        ...
        parent.save()
        topic = Wiki(parent=parent, vote_count=0, title=title,)
        topic.save()
