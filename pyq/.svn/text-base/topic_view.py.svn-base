...
def topic_handler(request, post_id):
    if request.method == "POST":
        pass
    elif request.method == "GET":
        return post(request, post_id)
    elif request.method == "PUT":
        return edit_topic(request, post_id)
    elif request.method == "DELETE":
        return delete_topic(request, post_id)
...
@login_required
def delete_topic(request, post_id):
    request_dict = {}
    if request.user.has_perm("topic.delete_topic"):
...
@login_required
def edit_topic(request, post_id):
    response_dict = {}
    if request.method == "PUT":
        post = Topic.objects.get(id=post_id)
        if request.user.has_perm("topic.edit_topic"):
...
def post_topic(request):
     if request.method == "POST":
        ...
        parent.save()
        topic = Topic(parent=parent, vote_count=0, title=title,)
        topic.save()
