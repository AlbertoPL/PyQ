...
def http_handler(request, post_id):
    if request.method == "POST":
        ...
        return post_static(request, post_id)
    elif request.method == "PUT":
        return edit_static(request, post_id)
    elif request.method == "DELETE":
        return delete_static(request, post_id)
...
@login_required
def delete_static(request, post_id):
    request_dict = {}
    if request.user.has_perm("staticquestion.delete_static"):
...
@login_required
def edit_static(request, post_id):
    response_dict = {}
    if request.method == "PUT":
        post = StaticQuestion.objects.get(id=post_id)
        if request.user.has_perm("staticquestion.edit_static"):
